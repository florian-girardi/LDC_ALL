...
<p><strong> Lineups have been added to the history and are available to consult in the Sharepoint</strong></p>
...
LDCDataAccessLayerPy.mail.mail_send(to=adrss, subject=f'South America Lineups {datetime.now().strftime("%d-%m")}',from_addr="florian.girardi-ext@ldc.com",body=body_1,mime_type="html")
...
def update_history(history, lineups_merged, shippers_mapping, sp_mgr):
  """
  Processes history and lineups_merged DataFrames, handling Flag and date filtering.

  Args:
      history: DataFrame containing historical data.
      lineups_merged: DataFrame containing merged lineups data.
      shippers_mapping: Dictionary for mapping shipper names.
      sp_mgr: Object for managing file operations (assumed).
  """

  # Clean and format columns
  history['Shipper'] = history['Shipper'].str.upper().apply(lambda x: shippers_mapping.get(x, x) if pd.notna(x) else x)
  history['Coordinator'] = history['Coordinator'].str.upper().apply(lambda x: shippers_mapping.get(x, x) if pd.notna(x) else 'UNKNOWN')
  history['Coordinator'] = history['Coordinator'].apply(lambda x: x if isinstance(x, str) else 'UNKNOWN')
  history['Shipper'] = history['Shipper'].apply(lambda x: x if isinstance(x, str) else 'UNKNOWN')
  history.loc[(history['Quantity'] == 'tbc'), ['Quantity']] = np.nan
  history = history.drop_duplicates()
  ...
  # Get current date and month start logic
  now = datetime.now()
  current_month_start = (now.replace(day=1) - timedelta(days=1)).replace(day=1) if now.day < 7 else now.replace(day=1)

  # Filter history (keep rows with Flag containing "x" or dates before current month)
  history_filtered = history[
      (history['Flag'].str.contains('x')) | (history['Date'] < current_month_start)
  ]

  # Group history by relevant columns (assuming 'Vessel' is the vessel name)
  grouped_history = history_filtered.groupby(['Vessel', 'Shipper', 'Coordinator'])

  # Apply a function to update each group with the latest date
  def update_group(group):
    return group.iloc[group['Date'].argmax()]  # Keep row with latest date

  # Update history with the latest dates
  updated_history = grouped_history.apply(update_group).reset_index()

  # Filter lineups_merged (current month onwards)
  lineups_current_and_after = lineups_merged[lineups_merged['Date'] >= current_month_start]

  # Combine filtered DataFrames
  combined_df = pd.concat([updated_history, lineups_current_and_after])
  ...
  sp_mgr.rm('/sites/GRP-TradingLineups/Lineups/merged/history and lineups.xlsx')
  sp_mgr.save_pd_to_excel('/sites/GRP-TradingLineups/Lineups/Merged/history and lineups.xlsx', combined_df, index=False)

# Call the function
update_history(history, lineups_merged, shippers_mapping, sp_mgr) '''
